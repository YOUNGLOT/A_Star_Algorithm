...
public class AStarTracking {
    ...
    public AStarTracking(int[][] goalArray, int[][] inputArray) {
        //  목표 Array
    ...
    public static void main(String[] args) {
        ...
        AStarTracking aStarTracking = new AStarTracking(goalArray, inputArray);
        aStarTracking.solve();
    ...
    private void solve() {
        ...
        //  결과를 프린트 (풀이 과정)
        printProcess(Long.toString(resultKey));
    }
    ...
    private void logic() {
        ...
        Map<Long, int[][]> map_In_TriageScore = queueMap.get(minTriageScore);

        //  for 문을 위한 KeySet
        //  clone 이유 : 향상된 for 문은 iterator 를 이용하기 때문에 동기화를 지원한다.
        Set<Long> clonedKeySet = cloneSet(map_In_TriageScore.keySet());

        //
        for (Long key : clonedKeySet) {
            //  답이 나왔을 때 탈출 조건
            if (resultKey != -1) { break; }
    ...
    private void printProcess(String resultKey) {
        ...
        */
        ...
        //  현재 result 출력
        printKeyArray(resultKey, resultArray);

        //  시작 Array의 키값을 1로 지정함 -> 1에서 멈춤
        if (resultKey.length() == 1) {
            return;
        } else {
            //  resultArray 필드 사용  ( 재귀함수 매개변수 사용 최소화 )
